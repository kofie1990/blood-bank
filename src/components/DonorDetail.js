...
const DonorDetail = () => {
  ...
  const donorDonations = donations.filter((d) => d.donor === id);
  if (!donor) {
    return <div>Donor not found</div>;
  }

  return (
    <div className="h-screen w-screen bg-cover bg-center bg-gradient-to-br from-red-300 to-blue-100 p-4 sm:p-6 overflow-hidden">
      {" "}
      <motion.div
